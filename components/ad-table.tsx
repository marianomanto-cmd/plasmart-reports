  ...
  return (
    <Card className="p-0">
      <div className="flex items-baseline justify-between border-b border-border-default px-4 py-4 sm:px-6">
        <h3 className="eyebrow-xs">Detalle de ads</h3>
      ...
      <div className="sm:hidden">
        ...
        <ul className="divide-y divide-border-soft">
          {sorted.map((row) => (
            <MobileAdCard key={row.adId} row={row} />
          ))}
        </ul>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border-default text-[10px] uppercase tracking-[0.18em] text-light">
              <Th k="adName"      sortKey={sortKey} sortDir={sortDir} onClick={onHeaderClick} align="left">Ad</Th>
            ...
            {sorted.map((row) => (
              ...
              >
                <td className="px-4 py-3 text-primary">
                  <span className="block max-w-[160px] truncate sm:max-w-[220px]" title={row.adName}>
                    {row.adName}
                  </span>
                  <span className="block text-[10px] uppercase tracking-[0.1em] text-light">
                    {row.publisher === "gads" ? "Google Ads" : "Meta Ads"}
                  </span>
                </td>
                <td className="px-4 py-3 text-steel">
                  <span className="block max-w-[140px] truncate sm:max-w-[200px]" title={row.adsetName}>
                    {row.adsetName}
                  </span>
                </td>
                <td className="px-4 py-3 text-steel">
...
function MobileAdCard({ row }: { row: AdRow }) {
  return (
    <li className="px-4 py-4">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <p className="truncate text-sm font-semibold text-primary" title={row.adName}>
            {row.adName}
          </p>
          <p className="mt-0.5 truncate text-[11px] text-steel" title={row.adsetName}>
...
function EmptyState() {
  return (
    <Card>
      <h3 className="mb-4 eyebrow-xs">Detalle de ads</h3>
      <div className="flex h-[200px] flex-col items-center justify-center gap-2 text-center text-sm text-light">
        <p>Sin datos de ads en el rango y filtros seleccionados.</p>
        <p className="text-xs">
          Recordá que la ingesta de ads está disponible sólo para Google Ads
